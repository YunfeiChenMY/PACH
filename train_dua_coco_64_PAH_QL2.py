...
class Session:
    ...
    def eval2(self, l1, l2, l3, l4, l5, l6, l7, database_loader):
        # self.logger.info('--------------------Evaluation: Calculate top MAP-------------------')
        ...
        if settings.DATASET == "WIKI":
            re_BI, re_BT, re_L, qu_BI, qu_BT, qu_L = compress_wiki(database_loader, self.test_loader,
                                                                   self.CodeNet_I, self.CodeNet_T, self.database_dataset,
                                                                   self.test_dataset)

        if settings.DATASET == "MIRFlickr" or settings.DATASET == "NUSWIDE" or settings.DATASET == "MSCOCO":
            re_BI, re_BT, re_L, qu_BI, qu_BT, qu_L = compress(database_loader, self.test_loader, self.CodeNet_I,
                                                              self.CodeNet_T,
                                                              self.database_dataset, self.test_dataset)
...
def main():
    ...
    if settings.DATASET == "MSCOCO":
        # dataloader, data_train = get_loader_nus(settings.BATCH_SIZE)
        dataloader, data_train, dataloader2 = get_loader_coco_CLIP(settings.BATCH_SIZE)
        train_dataset = dataloader['train']
        test_dataset = dataloader['query']
        database_dataset = dataloader['database']
    ...
    for i in range(1000000):
        ...
        max_e = 100
        observation = torch.ones(1, n_features)
        observation = torch.Tensor([[10.000000, 0.010000, 1.000000, 0.100000, 0.010000]])
        # observation[0, 5] = 0.5
            ...
            if state not in q_table.index:
                ...
                for epoch in range(settings.NUM_EPOCH):
                    ...
                    if (epoch + 1) % settings.EVAL_INTERVAL == 0:
                        if epoch < 200:
                            B = sess.eval2(l1, l2, l3, l4, l5, l6, l7, dataloader2['database'])
                            # B = sess.eval2(l1, l2, l3, l4, l5, l6, l7, dataloader['database'])
                        else:
                        ...
                        #     break
                        # print(A)
                        if (B < 1.72 and epoch > 15) or (B < 1.67 and epoch < 15):
                            break
                end_time = time.time()
                duration = end_time - start_time
                print(f"程序执行时间：{duration}秒")
